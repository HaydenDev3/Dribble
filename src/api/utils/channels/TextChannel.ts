...
export class TextChannel extends GuildChannel implements TextBasedChannel {
  ...
  async send (content: MessageOptions | string) {
    if (typeof content === "string") {
      ...
      const response = await this.client.rest.createMessage(body, this.id) as any;
      response.guild_id = this.guild.id;
      return await buildMessage(this.client, response);
    }

    if (content instanceof MessageEmbed) {
      const options: MessageOptions = content as any;
      const response = await this.client.rest.createMessage(options, this.id) as any;
      response.guild_id = this.guild.id;
      return await buildMessage(this.client, response);
    }

    const response = await this.client.rest.createMessage(content, this.id) as any;
    response.guild_id = this.guild.id;
    return await buildMessage(this.client, response);
  }
